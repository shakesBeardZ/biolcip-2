...
def get_coral_split_dataset(args, preprocess_fn, is_train, epoch=0, tokenizer=None):
    if CoralTrainingDataset is None:
        raise ImportError("CoralTrainingDataset not available; ensure dataset_train.py is importable")

    csv_path = None
    if is_train:
        csv_path = args.train_data
    else:
        csv_path = args.val_data or args.train_data
    if not csv_path:
        raise ValueError("--train-data must be provided when using dataset-type coral_split")

    split_values = args.coral_train_split if is_train else args.coral_val_split
    include_splits = [s.strip() for s in split_values.split(',') if s.strip()]
    if not include_splits:
        raise ValueError("No valid split values provided for coral_split dataset")

    caption_level = args.coral_caption_level or args.coral_target_level

    base_dataset = CoralTrainingDataset(
        csv_file=csv_path,
        transform=preprocess_fn,
        target_level=args.coral_target_level,
        rank_only=args.coral_rank_only,
        caption_level=caption_level,
        caption_mode=args.coral_caption_mode,
        normalize_anthozoa=not args.no_coral_normalize_anthozoa,
        path_key=args.coral_path_key,
    ...
    if tokenizer is None:
        raise ValueError("Tokenizer must be provided for coral_split dataset")

    class CoralWrappedDataset(Dataset):
        def __init__(self, base_ds, token_fn):
            self.base = base_ds
            self.tokenize = token_fn
    ...
    dataset = CoralWrappedDataset(base_dataset, tokenizer)
    num_samples = len(dataset)
...
def get_dataset_fn(data_path, dataset_type):
    ...
    elif dataset_type == "coral_split":
        return get_coral_split_dataset
...
def get_data(args, preprocess_fns, epoch=0, tokenizer=None):
    ...
    if args.train_data or args.dataset_type in ("synthetic", "coral_split"):
        data["train"] = get_dataset_fn(args.train_data, args.dataset_type)(
            args, preprocess_train, is_train=True, epoch=epoch, tokenizer=tokenizer)

    if args.val_data or args.dataset_type == "coral_split":
