...
def parse_args(args):
    ...
    parser.add_argument(
        "--dataset-type",
        choices=["webdataset", "csv", "synthetic", "auto", "coral_split"],
        default="auto",
    ...
    parser.add_argument(
        ...
        type=str,
        default="split",
        help="Column in the CSV identifying dataset split when using coral_split dataset type."
    )
    parser.add_argument(
        "--coral-train-split",
        type=str,
        default="train",
        help="Value(s) in split column for training rows (comma separated)."
    )
    parser.add_argument(
        "--coral-val-split",
        type=str,
        default="val",
        help="Value(s) in split column for validation rows (comma separated)."
    )
    parser.add_argument(
        "--coral-target-level",
        type=str,
        default="species",
        choices=["kingdom","phylum","cls","order","family","genus","species"],
        help="Taxonomic rank to use when constructing labels for coral_split dataset."
    )
    parser.add_argument(
        "--coral-caption-level",
        type=str,
        default=None,
        help="Taxonomic rank to use for captions (defaults to coral-target-level)."
    )
    parser.add_argument(
        "--coral-caption-mode",
        type=str,
        ...
        help="Caption composition mode for coral_split dataset."
    )
    parser.add_argument(
        "--coral-rank-only",
        action="store_true",
        help="If set, produce rank-only labels from coral dataset (otherwise use chain)."
    )
    parser.add_argument(
        "--no-coral-normalize-anthozoa",
        action="store_true",
        help="Disable Anthozoa normalization in coral_split dataset."
    )
    parser.add_argument(
        "--coral-keep-missing-targets",
        action="store_true",
        help="Keep samples even if the requested coral taxonomy rank is missing (default: drop them)."
    )
    parser.add_argument(
        "--coral-path-key",
        type=str,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Leave the text projection unfrozen when locking the text tower."
    )
    parser.add_argument(
        "--text-proj-lr",
        type=float,
        default=None,
        help="Optional learning rate override for text projection parameters."
    )
    parser.add_argument(
        "--imagenet-val",
        type=str,
