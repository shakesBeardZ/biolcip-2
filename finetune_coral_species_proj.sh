#!/bin/bash
#SBATCH --job-name=bioclip2-coral-species-proj
#SBATCH --nodes=1
...
nvidia-smi
# Load a CUDA module available on this node and cuDNN if present
if command -v module &>/dev/null; then
  module purge
  module load cuda/12.4.1 || module load cuda/12.2 || module load cuda/12.1 || module load cuda/11.8 || module load cuda/11.7.1 || true
  module load cudnn || true
fi

RUN_NAME=${RUN_NAME:-"coral_species_proj_ft_$(date +%Y%m%d_%H%M%S)"}
LOG_DIR=${LOG_DIR:-"./logs_finetune"}
CSV_PATH=/home/yahiab/reefnet_project/yahia_code/data_preprocessing_scripts/reefnet.ai/inaturalist/inat_cotw_training_split_fixed_paths_cleaned_final.csv
