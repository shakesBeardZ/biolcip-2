#!/bin/bash
#SBATCH --job-name=bioclip2-ft-genus-chain-proj
#SBATCH --nodes=1
...
if command -v module &>/dev/null; then
  module purge
  module load cuda/11.8 || module load cuda/12.2 || module load cuda/12.1 || module load cuda/12.4.1 || module load cuda/11.7.1 || true
fi

RUN_NAME=${RUN_NAME:-"ft_genus_chain_proj_$(date +%Y%m%d_%H%M%S)"}
